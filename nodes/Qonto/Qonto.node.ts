...
// ------------------------
								...
								if (resource === 'attachment') {
										if (operation === 'uploadAttachment') {
												...
												const idempotency_key = this.getNodeParameter('idempotency_key', i) as string;
												const attachment_ids = this.getNodeParameter('attachment_ids', i) as IDataObject;
...
// ------------------------
								...
								if (resource === 'requests') {
										...
										if (operation === 'approveARequest') {
												const id = this.getNodeParameter(`id`, i) as string;
												const request_type = this.getNodeParameter(`request_type`, i) as string;
												const endpoint = `requests/${request_type}/${id}/approve`;
												const idempotency_key = this.getNodeParameter('idempotency_key', i) as string;
												headers = {'X-Qonto-Idempotency-Key': idempotency_key};
												const debit_iban = this.getNodeParameter('debit_iban', i) as string;
												body.debit_iban = debit_iban;
										...
										if (operation === 'declineARequest') {
												const id = this.getNodeParameter(`id`, i) as string;
												const request_type = this.getNodeParameter(`request_type`, i) as string;
												const endpoint = `requests/${request_type}/${id}/decline`;
												const idempotency_key = this.getNodeParameter('idempotency_key', i) as string;
												headers = {'X-Qonto-Idempotency-Key': idempotency_key};
												const declined_note = this.getNodeParameter('declined_note', i) as string;
												body.declined_note = declined_note;
